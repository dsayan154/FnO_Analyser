...
```bash
fno_data_provider$ sam build --use-container
```
...
```bash
fno_data_provider$ sam logs -n StockCheckerFunction --stack-name fno_data_provider --tail
```
...
```bash
fno_data_provider$ pip install -r tests/requirements.txt --user
# unit test
fno_data_provider$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
fno_data_provider$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name fno_data_provider
```
